...
class TestSMACrossover(unittest.TestCase):
    def setUp(self):
        date_range = pd.date_range(start="2025-02-01", end="2025-02-10", freq="T")
        price_data = pd.DataFrame(
            {"close": [100 + i for i in range(len(date_range))]}, index=date_range
        )
...
class TestRSIBBStrategy(unittest.TestCase):
    def setUp(self):
        ...
        index = pd.date_range(start="2022-01-01", periods=len(data["close"]), freq="1T")
...
class TestVWAPReversionStrategy(unittest.TestCase):
    def setUp(self):
        self.data = {
            ...
            "volume": [1000, 1200, 1100, 1150, 1180, 1200, 1300, 1250, 1270, 1300],
        }
